// im not merging this so wahtever
...
/obj/machinery/computer/atmos_sim/proc/gas_topic(datum/gas_mixture/mix, href_list)
	...
	if(href_list["change_volume"])
		...
		switch(volume_type)
			if("Custom")
				...
				if(!src || !usr || !usr.canUseTopic(src) || QDELETED(src) || desired_volume == null)
					return
			if("Floor Tile")
	...
	if(href_list["change_temperature"])
		...
		if(!src || !usr || !usr.canUseTopic(src) || QDELETED(src) || new_temp == null)
			return
...
/obj/item/circuitboard/computer/atmos_sim
	name = "Atmospherics Simulator (Computer Board)"
